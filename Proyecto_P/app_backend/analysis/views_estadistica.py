...
class EstadisticasPesquerasView(APIView):
    """
    Genera estadísticas pesqueras basadas en niveles de desembarque y esfuerzo pesquero, con filtros avanzados.
    """
    ...
    def obtener_resumen_general(self, capturas_query, avistamientos_query, incidencias_query, lances_query):
        """
        Genera el resumen general de las estadísticas.
        """
    ...
    def obtener_capturas_mas_comunes(self, capturas_query):
        """
        Obtiene las capturas más comunes.
        """
        return capturas_query.values('especie__nombre_cientifico').annotate(
    ...
    def obtener_tendencia_lances(self, lances_query):
        ...
        return lances_query.annotate(year=ExtractYear('actividad__fecha_salida')).values('year').annotate(
            total_lances=Count('pk'),  # Usando 'pk' para contar la clave primaria
            promedio_profundidad=Avg('profundidad_suelo_marino')
        ).order_by('year')
    ...
    def get_filtros_aplicados(self, filtros):
        ...
        """
        filtros_aplicados = {}
        if filtros.get('profundidad_min'):
            filtros_aplicados['Profundidad Mínima'] = filtros.get('profundidad_min')
        else:
            filtros_aplicados['Profundidad Mínima'] = "No especificada"

        if filtros.get('profundidad_max'):
            filtros_aplicados['Profundidad Máxima'] = filtros.get('profundidad_max')
        else:
            filtros_aplicados['Profundidad Máxima'] = "No especificada"

        if filtros.get('embarcacion'):
            filtros_aplicados['Embarcación'] = filtros.get('embarcacion')
        else:
            filtros_aplicados['Embarcación'] = "Todas"

        if filtros.get('mes_captura'):
            filtros_aplicados['Mes de Captura'] = filtros.get('mes_captura')
        else:
            filtros_aplicados['Mes de Captura'] = "Todos"

        if filtros.get('ano_captura'):
            filtros_aplicados['Año de Captura'] = filtros.get('ano_captura')
        else:
            filtros_aplicados['Año de Captura'] = "Todos"
    ...
    def get(self, request):
        try:
            ...
            # Generar estadísticas de niveles de desembarque
            niveles_desembarque = self.obtener_niveles_desembarque(capturas_query)

            # Construir consulta de esfuerzo pesquero con filtros
            ...
            # Generar estadísticas de esfuerzo pesquero
            esfuerzo_pesquero = self.calcular_esfuerzo_pesquero(esfuerzo_pesquero_query)

            # Generar resumen general
            # Asumiendo que existen los modelos Avistamiento e Incidencia
...
class ListFiltroView(APIView):
    ...
    def get(self, request):
        try:
            ...
            embarcaciones_list = [
                emb['actividad__embarcacion__nombre_embarcacion'] for emb in embarcaciones_con_datos if emb['actividad__embarcacion__nombre_embarcacion']
            ]
